...
class ImageDataSet(RandomImageDataSet):
    def __init__(self, img: np.array, num_sample: float):
        ...
        """
        super().__init__(img, num_sample=num_sample)
        #get coords in N x 2 in (r, c) format
        self.coords = np.indices(img.shape[:2]).reshape(2, -1).T
        
    def __len__(self):
        return (self.coords.shape[0]) // self.num_sample
    def __getitem__(self, idx):
        batch_coords = self.coords[idx * self.num_sample : (idx + 1) * self.num_sample, ::-1]
        norm_coords = self.normalize_coords(batch_coords)
        clr = self.image[batch_coords[:, 1], batch_coords[:, 0]]
        return torch.from_numpy(norm_coords).float(), torch.from_numpy(clr).float()
class ImageDataLoader(object):
    ...
    def add_img(self, img: np.array):
        ...
        img_loader = DataLoader(img_dataset, batch_size=self.sample_size, shuffle=True)
